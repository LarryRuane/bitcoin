...
{
    size_t size{0};
    for (const auto& coins : coins_by_type) {
        size += coins.size();
    }
    return size;
}
...
std::vector<COutput> CoinsResult::All() const
{
    ...
    for (const auto& coins : coins_by_type) {
        all.insert(all.end(), coins.begin(), coins.end());
    }
    return all;
}
                               ...
                               const CoinSelectionParams& coin_selection_params, bool allow_mixed_output_types)
{
    ...
    for (const auto& coins : available_coins.coins_by_type) {
        if (auto result{ChooseSelectionResult(wallet, nTargetValue, eligibility_filter, coins, coin_selection_params)}) {
