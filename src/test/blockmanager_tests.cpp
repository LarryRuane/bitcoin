...
BOOST_AUTO_TEST_CASE(blockmanager_find_block_pos)
{
    ...
    FlatFilePos pos;
    {
        ...
        pos = blockman.SaveBlockToDisk(params->GenesisBlock(), 0, chain, *params, nullptr);
        BOOST_CHECK_EQUAL(pos.nFile, 0);
        BOOST_CHECK_EQUAL(pos.nPos, BLOCK_SERIALIZATION_HEADER_SIZE);
    }
    // Simulate a node restart (fresh blockman, for reindex, which always happens during startup).
    BlockManager blockman;
    CChain chain;
    ...
    BOOST_CHECK_EQUAL(pos.nFile, 0);
    BOOST_CHECK_EQUAL(pos.nPos, (2 * BLOCK_SERIALIZATION_HEADER_SIZE) + ::GetSerializeSize(params->GenesisBlock(), CLIENT_VERSION));
}
